...
def feedback_shame_table(base_url = BASE_URL, round = 'r/1') -> pd.DataFrame:
    ...
    df_submit_compare = df_should_submit.set_index(['from', 'to']).join(df_feedback_given).groupby('from').agg({'count': 'sum', 'source': 'count'}).rename(columns={'count': 'should', 'source': 'given'})
